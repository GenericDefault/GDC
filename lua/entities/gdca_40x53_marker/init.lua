 ...
 function ENT:Think()
	...
	local tr = util.TraceLine( trace )
				...
				if tr.Hit then
					...
					util.BlastDamage(self.Entity, self.Entity, tr.HitPos, 50, 20)
					local effectdata = EffectData()
					...
					util.Effect( "gdca_smokepuff", effectdata )
	...
	self.flightvector = self.flightvector + Vector(math.Rand(-0.2,0.2), math.Rand(-0.2,0.2),math.Rand(-0.2,0.2)) + Vector(0,0,-0.111)
