...
self.Penetrate 		= 40					// How deep Generic Default goes (in yo mama)
self.Flightvector 	= self.Entity:GetUp()*200		// Velocity in inches/sec
...
self.BlastDamage	= 300					// How much it dislodges your small intestine
self.BlastRadius	= 300					// How big impact radius is
...
function ENT:Think()
	...
	trace.endpos 	= self.Entity:GetPos() + self.Flightvector*1.3	// Trace to where it will be next tick
	...
	local tr = util.TraceLine( trace )
		...
		util.BlastDamage(self.Entity, self.Entity, tr.HitPos, self.BlastRadius, self.BlastDamage)	// Radius, Damage
			...
			local attack =  gdc.caphit( tr.Entity, 1000) 	// Entity, Damage
			end
			///
